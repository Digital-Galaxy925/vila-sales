...
const UploadST = () => {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        ...
        .single();
      if (data) {
        setFile({ name: data.file_name, size: 0 } as File);
  ...
  const processFile = useCallback(async (f: File) => {
    ...
    try {
      ...
      const { error } = await supabase.from("st_data").insert({
        data: rows as any,
        file_name: f.name,
      ...
      console.error(err);
      toast.error("Erro ao salvar dados de ST");
    } finally {
      setSaving(false);
    }
  }, []);
  ...
  const removeFile = async () => {
    ...
    setLoaded(false);
    setRowCount(0);
    toast.success("Dados de ST removidos");
  };
    ...
    <div>
      ...
      >
        ...
        <Button variant="outline" className="pointer-events-none">
          {saving ? "Salvando..." : "Selecionar Arquivo"}
        </Button>
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6 space-y-3">
          <div className="flex items-center justify-between bg-card rounded-xl p-4 shadow-[var(--shadow-card)]">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={(e) => { e.stopPropagation(); removeFile(); }} className="text-muted-foreground hover:text-destructive transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
