    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
        ...
        <div onClick={() => setFilterMarg(filterMarg === "critico" ? "all" : "critico")} className="cursor-pointer">
          <KpiCard label={`Críticos (< ${minMargin}%)`} value={String(criticos)} sub={`${buBase.length ? ((criticos/buBase.length)*100).toFixed(0) : 0}% do mix${filterMarg === "critico" ? " • filtro ativo" : ""}`} color="#f87171" icon="🚨" /> value={String(criticos)} sub={`${buBase.length ? ((criticos/buBase.length)*100).toFixed(0) : 0}% do mix${filterMarg === "critico" ? " • filtro ativo" : ""}`} color="#f87171" icon="🚨" />
        </div>
